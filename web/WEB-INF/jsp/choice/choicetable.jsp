...
<div id="page-wrapper">
	<div class="row">
		<div class="col-lg-12">
			<h4 style="text-align: center">Choice Manger</h4>
		</div>
		<!-- /.col-lg-12 -->
	</div>
	<!-- /.row -->
	...
	<script>
		$(document).ready(function() {
			...
			var cols = [
				...
				{ "data": "no" , "orderable" : true}, // 设置第一列
				{ "data": "description" , "orderable" : false}, // 设置第二列
				{ "data": "questionId" , "orderable" : false}, // 设置第二列
				// 下面在操作列中添加了修改和删除按钮
			...
			var table = $('#' + tableId).DataTable({
				...
				"rowId" : 'id',// 设置主键字段名称
				"ajax": {
					"url": "${pageContext.request.contextPath}/exam/choice/list", // 请求列表数据的url
					"type": "GET" // http方法
				},
			...
			$('#' + tableId + ' tbody').on( 'click', 'tr', function () {
				// 切换样式selected，如果有selected样式则删除，没有则添加
				$(this).toggleClass('selected');
			} );

			// 添加工具栏按钮，找到设置分页的div，并添加创建和批量删除按钮的a标签
			$('#' + tableId + '_length').append (" <a class='btn btn-primary btn-sm' onclick='createExamChoice();'>创建</a> <a class='btn btn-primary btn-warning btn-sm' onclick='deleteExamChoices();'>批量删除</a>");
		});

		function createExamChoice () {
			location.href = "${pageContext.request.contextPath}/exam/choice/create";
		}

		function updateExamChoice (id) {
			location.href = "${pageContext.request.contextPath}/exam/choice/update?id=" + id;
		}
		...
		function deleteExamChoice (id) {
			if (!confirm ('确定要删除选定的选项吗？')) {
				...
				location.href = "${pageContext.request.contextPath}/exam/choice/delete1?id=" + id;
			}
		}
		...
		function deleteExamChoices () {
			...
			$.post ('${pageContext.request.contextPath}/exam/choice/delete', {id : ids}, function (result) {
				if (result.success) {
					var table = $('#tableExamChoice').DataTable();
					// 删除选中的行
