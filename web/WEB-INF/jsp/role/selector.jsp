	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
            ...
            <div class="modal-body">
                <table width="100%" class="table table-striped table-bordered table-hover" id="tableSysRoleSelector">
                    <thead>
                        <tr>
        ...
        <script>
        	// 构造DataTable
			var table = $('#tableSysRoleSelector').DataTable ({
				"processing": true, // 显示处理进度
                "serverSide": true, // 从服务端获取数据
                "rowId" : 'id',// 设置主键字段名称
                "ajax": {
                    "url": "${pageContext.request.contextPath}/sys/role/selector/list", // 请求列表数据的url
                    "type": "GET" // http方法
                },
                // "order": [ [ 1, "desc" ] ] , // 排序
                "columns": [
        	...
        	table.on( 'draw', function () {
        		$('#tableSysRoleSelector tbody tr').each (function () {
            ...
            $('#tableSysRoleSelector tbody').on( 'click', 'tr', function () {
			...
			// 增加确定和取消按钮
			$('#tableSysRoleSelector_length').append(" <a class='btn btn-primary btn-sm'>确定</a>" 
				+ " <a class='btn btn-primary btn-warning btn-sm'>取消</a>");
			// 确定按钮点击事件
			$('#tableSysRoleSelector_length a:first').click (function () {
				// 保存选中数据
				idField.val(selectedData.id.join(','));
				nameField.val(selectedData.name.join(','));
				// 隐藏对话框
				$('#tableSysRoleSelector').parents ('.modal').modal('hide');
			});
			// 取消按钮点击事件
			$('#tableSysRoleSelector_length a:last').click (function () {
				// 隐藏对话框
				$('#tableSysRoleSelector').parents ('.modal').modal('hide');
			});
		</script>
